...
const fastStream = [
  { source: 'TG Bot', event: 'Secret Party', status: 'Успешно' },
  { source: 'VK Parser', event: 'Art Lecture', status: 'Успешно' },
  { source: 'Сайты', event: 'Jazz Night', status: 'Успешно' },
  { source: 'Партнерские API', event: 'Marathon', status: 'Успешно' },
  { source: 'User Bot', event: 'Standup Open Mic', status: 'Успешно' },
  { source: 'TG Channel', event: 'Tech Meetup', status: 'Успешно' },
  { source: 'Instagram', event: 'Pop-up Market', status: 'Успешно' },
]
...
<template>
  <section ref="root" class="slide-container relative min-h-screen w-full flex flex-col items-center justify-center p-6 bg-[var(--bg-color)] overflow-x-hidden">
    ...
    <div class="text-center mb-10 z-10">
      <p class="text-sm uppercase tracking-[0.3em] text-[var(--primary)] font-bold mb-3">Наше главное преимущество</p>
      <h2 class="text-5xl md:text-6xl font-black uppercase tracking-tight text-[var(--text-color)]">
        полный оховат<span class="text-transparent bg-clip-text bg-gradient-to-r from-[var(--primary)] to-[var(--secondary)]">событий.</span>
      </h2>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-6xl h-auto md:h-[500px]">
      ...
      <article class="advantage-card relative rounded-3xl border-2 border-gray-200 bg-gray-50 overflow-hidden flex flex-col p-6 md:p-8 shadow-inner group transition-all duration-500">
        <div class="absolute top-6 right-6 px-3 py-1 bg-gray-200 text-gray-500 text-sm font-bold uppercase rounded-full">
          Конкуренты
        </div>
        ...
        <div class="mt-4">
          ...
          <h3 class="text-3xl font-bold text-gray-700">Традиционные афиши</h3>
          <p class="text-lg text-gray-500 mt-2 leading-relaxed">
            События добавляются вручную организаторами.
          </p>
        </div>

        <div class="mt-auto space-y-3 opacity-60">
           <div class="h-10 border border-gray-300 rounded-lg bg-white flex items-center px-3 text-lg text-gray-400">
           ...
           <div class="mt-4">
             <div class="flex justify-between text-sm uppercase font-bold text-gray-400 mb-1">
      ...
      <article class="advantage-card relative rounded-3xl border-2 border-[var(--primary)] bg-white overflow-hidden flex flex-col p-6 md:p-8 shadow-[0_20px_50px_rgba(0,0,0,0.1)] z-10">
        <div class="absolute top-6 right-6 px-3 py-1 bg-[var(--primary)] text-white text-sm font-bold uppercase rounded-full shadow-lg animate-pulse">
          Наш подход
        </div>

        <div class="relative z-20 mt-4 rounded-2xl bg-white/88 backdrop-blur-sm p-4 md:p-5 shadow-[0_12px_28px_rgba(0,0,0,0.08)]">
          ...
          <h3 class="text-3xl font-bold text-[var(--text-color)]">Мы собираем события автоматически.</h3>
          <p class="text-lg text-gray-600 mt-2 leading-relaxed font-medium">
            Система мониторит соцсети, сайты и сообщества и публикует новые события почти мгновенно.
          </p>
        </div>

        <div class="absolute inset-x-0 bottom-0 h-[42%] md:h-[56%] overflow-hidden z-10 mask-gradient-bottom pointer-events-none">
           <div class="absolute inset-x-0 top-0 h-14 bg-gradient-to-b from-white via-white/80 to-transparent z-20"></div>
           <div class="fast-stream-wrapper p-6 space-y-3 opacity-90 relative z-10">
             <div v-for="(item, i) in [...fastStream, ...fastStream]" :key="i" 
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <div class="text-sm font-bold text-[var(--primary)] uppercase">{{ item.source }}</div>
                    <div class="text-sm font-bold text-gray-800">{{ item.event }}</div>
                  </div>
                </div>
                <div class="text-sm font-black text-green-500 border border-green-200 px-1 rounded bg-green-50">
                   {{ item.status }}
                </div>
             </div>
           </div>
    ...
    <div class="grid grid-cols-2 gap-4 w-full max-w-4xl mt-8">
       <div class="text-center p-4 rounded-2xl bg-gray-50 border border-gray-200">
          <p class="text-sm font-bold text-gray-400 uppercase">Охват Конкурентов</p>
          <p class="text-4xl font-black text-gray-400 mt-1">~20%</p>
       </div>
       <div class="text-center p-4 rounded-2xl bg-white border-2 border-[var(--primary)] shadow-lg relative overflow-hidden">
          ...
          <p class="text-sm font-bold text-[var(--primary)] uppercase relative z-10">Наш Охват</p>
          <p class="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-[var(--primary)] to-[var(--secondary)] mt-1 relative z-10">100%</p>
       </div>
    </div>
