...
<template>
  <section ref="root" class="team-slide relative isolate min-h-screen w-full overflow-hidden bg-[var(--bg-color)] flex flex-col items-center justify-center p-6 md:p-12">
    ...
    <div class="team-title text-center mb-12 relative z-10">
      <p class="text-sm uppercase tracking-[0.3em] font-bold text-gray-400 mb-2">Execution Unit</p>
      <h2 class="text-5xl md:text-6xl font-black uppercase tracking-tighter text-[var(--text-color)]">
        КОМАНДА
      </h2>
    </div>
    ...
    <div class="team-grid grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-6xl relative z-10">
      ...
      >
        ...
        <div class="p-6 text-center">
          <div class="inline-block px-3 py-1 rounded-full text-sm font-black uppercase tracking-widest mb-3 text-white" :style="{ backgroundColor: member.color }">
            {{ member.role }}
          </div>
          
          <h3 class="text-3xl font-bold text-[var(--text-color)] mb-2 group-hover:text-[var(--primary)] transition-colors">
            {{ member.name }}
          </h3>
    ...
    <div class="mt-12 text-center max-w-2xl text-gray-400 text-lg font-medium relative z-10">
