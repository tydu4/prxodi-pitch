...
const solutionBlocks: SolutionBlock[] = [
  {
    title: 'Умный агрегатор (Парсинг 360°)',
    description:
      'Мы забираем данные сами. Наш поисковый движок мониторит сотни неструктурированных источников 24/7. Алгоритмы чистят мусор и выдают 100% покрытие города, включая андеграунд, который игнорируют крупные игроки.',
    mode: 'feed',
  },
  {
    title: 'Умные рекомендации (Нейросеть).',
    description:
      'Мы убили формат доски объявлений. Встроенная рекомендательная система и поиск единомышленников помогают найти компанию до выхода из дома. Покупка билета автоматически открывает доступ в закрытое комьюнити.',
    mode: 'map',
  },
  {
    title: 'Социальный капитал.',
    description:
      'Мы удерживаем пользователя на всех этапах. До ивента — матч по интересам, на входе — чекин в 1 клик, во время ивента — Live-стена для фото и нетворкинга с участниками, после — аналитика для организатора.',
    mode: 'friend',
  },
]

const perfectEvents: PerfectEvent[] = [
  { id: 1, title: 'Джаз на крыше', district: 'Патрики', time: '19:30' },
  { id: 2, title: 'Лекция про стартапы', district: 'Суперметалл', time: '18:45' },
  { id: 3, title: 'Прогулка по стрит-арту', district: 'Хлебозавод', time: '17:20' },
  { id: 4, title: 'Вечер настольных игр', district: 'Депо', time: '20:10' },
  { id: 5, title: 'Прогулка по стрит-арту', district: 'Бауманская', time: '16:55' },
  { id: 6, title: 'Джаз на крыше', district: 'Тверская', time: '21:00' },
]
  ...
  >
    ...
    <div class="relative z-10 mx-auto flex h-full w-full max-w-[1600px] flex-col justify-center px-6 pt-10 pb-6 md:px-12 md:pt-12 md:pb-8 lg:px-16">
      <h2 class="solution-title text-transparent bg-clip-text bg-gradient-to-r from-[var(--primary)] to-[var(--secondary)] max-w-[1100px] text-[clamp(1.6rem,3.2vw,3.4rem)] leading-[0.9] tracking-[-0.04em]">
        Единая платформа городских событий</h2>
      ...
      <div class="solution-layout mt-4 grid gap-5 md:mt-5 lg:grid-cols-5">
        <div class="phone-column lg:col-span-2">
          <div class="phone-stage">
            <article
              ...
              aria-label="Предпросмотр интеллектуального фильтра событий"
            >
                ...
                <div class="screen-body">
                  <section class="mode-layer feed-layer" :class="{ 'is-active': activeMode === 'feed' }">
                    <div class="feed-loop">
                      <article v-for="(eventItem, index) in feedLoop" :key="`${eventItem.id}-${index}`" class="event-card">
                        <p class="event-tag">Лучшее событие</p>
                  ...
                  <section class="mode-layer friend-layer" :class="{ 'is-active': activeMode === 'friend' }">
                    ...
                    <div class="friend-list">
                      <p>Общий интерес: Лекции о продуктах</p>
        ...
        <div class="copy-column flex flex-col gap-5 lg:col-span-3">
          <article
      ...
      <div class="print-grid mt-7">
        <article class="print-shot border-2 border-[--primary]">
          <p class="print-shot-title">Скриншот интерфейса платформы событий</p>
...
.solution-block.is-active {
  ...
  box-shadow: 0 20px 50px rgba(0, 0, 0, 0.08), 0 0 40px color-mix(in srgb, var(--secondary) 20%, transparent);
}

.block-title {
  font-family: 'Archivo Black', sans-serif;
  font-size: clamp(1.4rem, 2vw, 2.2rem); /* Увеличены минимальные и максимальные значения */
  line-height: 1.1;
...
.block-description {
  margin-top: 0.75rem; /* Добавлено больше воздуха между заголовком и текстом */
  font-size: clamp(1.1rem, 1.5vw, 1.6rem); /* Исправлен clamp для честной адаптивности */
  line-height: 1.45;
  font-weight: 500;
  color: var(--ink);
}
...
@media (max-width: 1023px) { .phone-shell { inline-size: min(64vw, 260px); } }
@media (max-width: 640px) {
  .friend-name { font-size: 0.9rem; }
  .friend-note { font-size: 0.76rem; line-height: 1.3; font-weight: 700; }
  .friend-list p { font-size: 0.8rem; line-height: 1.34; font-weight: 700; }
}
...
.solution-block {
    min-block-size: auto; /* Убираем жесткое ограничение 7.2rem */
    padding: 1.2rem; /* Увеличиваем внутренний отступ (было 0.85rem) */
  }

  .block-title {
    font-size: 1.3rem; /* Увеличиваем (было 1rem) */
  }

  .block-description {
    font-size: 1.1rem; /* Увеличиваем (было 0.8rem) */
    line-height: 1.4;
  }
}
